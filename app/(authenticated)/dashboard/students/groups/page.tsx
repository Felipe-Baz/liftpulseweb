...
export default function GroupsPage() {
  const { groups, addGroup, deleteGroup, students } = useStudents()
  ...
  const getStudentCount = (groupId: string) => {
    return students.filter((student) => student.groups.includes(groupId)).length
  }

  return (
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold">Grupos de Alunos</h1>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {groups.map((group) => (
            <div key={group.id} className="p-4 rounded-lg border relative" style={{ borderColor: group.color }}>
              ...
              <div className="text-sm text-muted-foreground">{getStudentCount(group.id)} alunos</div>
            </div>
          ))}
        </div>
      </div>
